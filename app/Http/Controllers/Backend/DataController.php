...
class DataController extends Controller
{
	...
	public function getClassEyesight(Request $request)
	{
			...
			// ->join('tbl_class','tbl_class.class_id','=','tbl_eyesight.class_id')
			// ->where([
			...
			// 	['tbl_class.class_year', '=', $request->class_year]
			// ])
			// ->select('tbl_eyesight.eyesight_diopter')
			// ->get();
		$query = "";
		if($request->yearIndex == 0)
		{
			$query = "SELECT tbl_eyesight.eyesight_diopter FROM tbl_school INNER JOIN tbl_academicyear ON tbl_school.school_id=tbl_academicyear.school_id
			INNER JOIN tbl_student ON tbl_student.academicYear_id=tbl_academicyear.academicYear_id
			INNER JOIN tbl_eyesight ON tbl_eyesight.stu_id = tbl_student.stu_id
		...
		else
		{
			...
			INNER JOIN tbl_student ON tbl_student.academicYear_id=tbl_academicyear.academicYear_id
			INNER JOIN tbl_eyesight ON tbl_eyesight.stu_id = tbl_student.stu_id
            INNER JOIN tbl_class ON tbl_class.class_id = tbl_eyesight.class_id
  ...
  //           ->join('tbl_eyesight', 'tbl_student.stu_id', '=', 'tbl_eyesight.stu_id')
		...
		if($request->yearIndex == 0)
		{
			$query = "SELECT * FROM `tbl_eyesight` WHERE tbl_eyesight.class_id = $request->classID AND tbl_eyesight.eyesight_diopter = $request->check";
		}
		else
		{
			$query = "SELECT * FROM `tbl_eyesight` JOIN tbl_class ON tbl_eyesight.class_id=tbl_class.class_id WHERE tbl_eyesight.class_id = $request->classID AND tbl_eyesight.eyesight_diopter = $request->check AND tbl_class.class_year='$request->class_year'";
		}
